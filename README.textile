...
Modifications and support by Arthur Brukish (abrukish@gmail.com)
...
Also, if you want to do something if form is valid/invalid just use callbacks: onFormValid and onFormInvalid. You could simply override them as follows:
<pre><code>Vanadium.onFormInvalid = function() { alert('Hmmm... Something went wrong.'); };</code></pre>

If form is invalid - it will scroll you up to the invalid element ( if you have scrollTo plugin installed of course ) by default.
...
Compatible with JQuery 1.7.1 (will be using other JS toolkits as well soon)
Tested in: Firefox 3.5 (PC/Mac), Safari 2 (Mac), IE 6+, Opera 9 (PC/Mac), Chrome (Win/Ubuntu)
